...
from analysis.backtest import load_data
...
def main(n_jobs, n_trials):
    print("\n--- [Stufe 2/2] Lokale Verfeinerung mit Optuna für mbot ---")
    ...
    for i, candidate in enumerate(candidates):
        print(f"\n===== Verfeinere Kandidat {i+1}/{len(candidates)} für {candidate['symbol']} =====")
    ...
    if best_overall_trial:
        ...
        final_params = {**best_overall_info['params'], **final_params_tuned, 
                        'start_capital': START_CAPITAL, 'leverage': 5.0,
                        'start_date_str': best_overall_info['start_date']}
        ...
        print(f"\n  >>> EINSTELLUNGEN FÜR DEINE 'mbot/config.json' <<<")
