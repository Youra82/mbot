...
def load_data(symbol, timeframe, start_date_str, end_date_str, required_days_before=50):
    ...
    if os.path.exists(cache_file):
        ...
        if data.index.min() <= download_start_dt.tz_localize('UTC') and data.index.max() >= pd.to_datetime(end_date_str).tz_localize('UTC'):
            full_data_in_range = data.loc[download_start_dt.strftime('%Y-%m-%d'):end_date_str]
            return full_data_in_range
