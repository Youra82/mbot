...
def run_mbot_backtest(data, params):
    ...
    for i in range(1, len(data)):
        ...
        if position:
            ...
            if not exit_price:
                ...
                elif position['side'] == 'short' and current_candle['low'] <= position['tp_price']:
                    exit_price, reason = position['tp_price'], "Take-Profit"
        ...
        if not position:
            long_entry = (
                ...
                current_candle['impulse_macd'] > 0 and current_candle['macd_uptrend'] == 1
            )
            short_entry = (
                current_candle['impulse_histo'] < 0 and prev_candle['impulse_histo'] >= 0 and
                current_candle['impulse_macd'] < 0 and current_candle['macd_uptrend'] == 0
            )

            trade_side = None
            if long_entry: trade_side = 'long'
            elif short_entry: trade_side = 'short'
    ...
    return {
        ...
        "params": params, "end_capital": current_capital, "max_drawdown_pct": max_drawdown_pct,
        "trade_log": trade_log
    }
